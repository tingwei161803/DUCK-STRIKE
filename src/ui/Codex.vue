...
import { modelThumbnail } from '../game/preview'
...
const enemyList = (Object.keys(ENEMIES) as EnemyId[]).map((id) => ({
  ...ENEMIES[id], desc: enemyDesc[id], url: `/models/characters/${ENEMIES[id].model}.gltf`,
}))
const weaponList = (Object.keys(WEAPONS) as WeaponId[]).map((id) => {
  ...
  return { ...w, tags, url: `/models/guns/${w.model}.gltf` }
})

// 素材縮圖（離線渲染，快取）
const thumbs = reactive<Record<string, string>>({})
onMounted(() => {
  const urls = [...new Set([...enemyList.map((e) => e.url), ...weaponList.map((w) => w.url)])]
  for (const url of urls) modelThumbnail(url).then((d) => { thumbs[url] = d })
})
</script>
  ...
  <div class="absolute inset-0 overflow-auto bg-black/85 backdrop-blur-sm z-40 flex flex-col items-center py-8 px-4">
    <div class="w-full max-w-2xl">
      ...
      <div v-if="tab === 'enemies'" class="space-y-2">
        <div v-for="e in enemyList" :key="e.id" class="bg-zinc-900/70 rounded-xl border border-white/10 p-3 flex gap-3 items-center">
          <div class="w-16 h-16 shrink-0 rounded-lg bg-black/40 overflow-hidden flex items-center justify-center">
            <img v-if="thumbs[e.url]" :src="thumbs[e.url]" class="w-full h-full object-contain" :alt="e.name" />
            <span v-else class="text-white/30 text-xs animate-pulse">…</span>
          </div>
      ...
      <div v-else class="space-y-2">
        <div v-for="w in weaponList" :key="w.id" class="bg-zinc-900/70 rounded-xl border border-white/10 p-3 flex gap-3 items-center">
          <div class="w-16 h-16 shrink-0 rounded-lg bg-black/40 overflow-hidden flex items-center justify-center">
            <img v-if="thumbs[w.url]" :src="thumbs[w.url]" class="w-full h-full object-contain" :alt="w.name" />
            <span v-else class="text-white/30 text-xs animate-pulse">…</span>
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between">
