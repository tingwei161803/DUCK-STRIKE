...
- **經濟**：擊殺/爆頭/連殺/過關給錢；**購買只在每波之間**（波次清空後自動開啟「軍火庫」買槍/補彈/護甲；戰鬥中無法購買）。
- **連殺獎勵**：連殺 5 → 回血、10 → 狂暴、15 → 彈藥全補。
- **背景音樂**：右上角選單，5 種程序合成曲風（緊張/輕快/史詩/電子/安靜），零音檔。
...
| 層 | 檔案 | 換新遊戲 |
|---|---|---|
| **引擎層（可重用）** | `game.ts` 主迴圈/命中解析/波次/爆炸/掉落/大絕時間縮放 · `player.ts` FPS 控制器（含震動/狂暴）· `weapons.ts` 武器系統 · `enemies.ts` AI+動畫池（含刀兵近戰/自爆/王/染色）· `grenades.ts` 手榴彈 · `projectiles.ts` 王彈幕 · `pickups.ts` 掉落物 · `map.ts` 地圖+爆炸桶 · `effects.ts` 打擊/爆炸特效 · `model-loader.ts` glTF 載入 · `input.ts` 輸入 · `sound.ts` 音效合成 · `music.ts` 程序背景音樂 | 幾乎不動 |
| **資料層（每款調）** | `config.ts` 的 `WEAPONS` / `ENEMIES` / `waveSpec` / `DIFFICULTIES` / `DROP` / `KILLSTREAK` / `GRENADE` / `ULTIMATE` / `PLAYER` / `ECONOMY` · `meta.ts` 的 `META_UPGRADES` · `music.ts` 的 `MUSIC_TRACKS` | 改資料陣列 |
...
- **盾兵**：正面擋彈、需繞背的敵人（角度判定）。
- 手機陀螺儀瞄準（需 HTTPS）、第三人稱切換、更多地圖、王的更多招式（震波/衝撞/毒池）。

## 已完成的主要系統
- 打擊感：鏡頭震動、武器晃動/sway、槍口火光/曳光錨定槍口、敵人可見曳光彈、傷害飄字、受擊方向指示器、可引爆爆炸桶、強化爆炸特效。
- 內容：無盡波次（隨波成長）、王波 + 王彈幕、刀兵（近戰衝鋒）、自爆兵、手榴彈、掉落物（補血/彈藥/狂暴）、限時強化。
- 系統：時間緩慢大絕（子彈時間）、5 種程序合成背景音樂、橫向手機觸控（射擊鈕兼瞄準）。
