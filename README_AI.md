...
- **已可玩、typecheck 乾淨**。已完成系統：
  - 核心：FPS 控制器（pointer-lock 鍵鼠 + **手機觸控**）、hitscan 命中（爆頭/LOS 遮蔽/散布/後座）、6 把武器、購買經濟。
  - 敵人：雜兵/突擊兵/重裝兵/**自爆兵**/**王**（動畫池 + 難度倍率 + 兵種染色 + 頭頂血條）。
  - 王招式：彈幕（環射/扇形）、血量≤35% 狂暴化、跨血量門檻召喚小兵。
  - 打擊感：鏡頭震動、武器晃動/sway、傷害/金錢飄字、受擊方向指示器、可引爆爆炸桶（連鎖）。
...
| `src/game/input.ts` | 鍵盤 + pointer-lock 滑鼠 + **觸控注入 API**（`setMove`/`addLook`/`setFire`/`setAim`/`queueJump`/`setCrouch`/`reloadPress`/`nextWeapon`） |
| `src/game/sound.ts` | Web Audio 程序合成 SFX（零音檔） |
| `src/game/meta.ts` | 永久升級 + 本地排行榜（localStorage，`Meta` 單例） |
| `src/game/config.ts` | **資料層**：所有數值表 |
| `src/ui/*.vue` | `HUD` / `BuyMenu`（軍火庫）/ `UpgradeMenu`（永久升級）/ `TouchControls`（手機虛擬控制） |
...
- **盾兵**：正面擋彈、需繞背（角度判定）。
- 手榴彈投擲、王的更多招式（震波/衝撞/毒池）、第三人稱切換、更多地圖。
